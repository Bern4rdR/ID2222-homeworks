...
def candidate(baskets, k, s, singletons):  # wip
    if k <= 1:
        return singletons
...
def count_kton_pool(baskets: list[set[int]], ktons: list[set[int]], threshold):
    passing_candidates = []
    with Pool(
        10, initializer=init_pool, initargs=(baskets,)
    ) as p:  # no need to go past number threads in cpu
...
def pipeline(baskets, threshold, max_k_ton):
    ...
    singletons = [
        i
        for i, x in enumerate(fast_count_items(baskets, count_init))
        if x / len(baskets) > threshold
    ]
    print(
        f"Found {len(singletons)} Singletons with support above threshold: {threshold}."
    )
...
def support(itemset: set, baskets: list[set]):
    return [basket for basket in baskets if itemset.issubset(basket)]
...
def main():
    ...
    candidate_ktons, singtons = pipeline(
        baskets=baskets, threshold=0.02, max_k_ton=4
    )  # 375 meet this thresh, 70125 candidate pairs for theshold 0.01

    time_start = perf_counter()
    rules = gen_rules(
        candidate_ktons, 5, 0.1, singtons, baskets
